...
@router.callback_query(F.data.startswith('exit_not_done'))
async def request_exit_not_done_callback_handler(query: CallbackQuery) -> None:
    ...
    result = await crm.change_repair_request_status(request_id, 5)
    
    if result:
        await query.message.answer('Статус заявки изменён на "Выезд без выполнения" ✅')
