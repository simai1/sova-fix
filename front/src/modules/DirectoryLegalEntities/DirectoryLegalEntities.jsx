...
import {
  ...
  GetlegalEntitiesOne,
  EditLegalEntities,
} from "../../API/API"; // Ensure CreateLegalEntity is imported
...
function DirectoryLegalEntities() {
  ...
  const handleCreateUnit = () => {
    if (
      ...
      !legalForm ||
      !startCoop ||
      (legalForm === "Другое" && !legalFormEcho)
    ) {
    ...
    popUpEdit
      ? EditLegalEntities(newUnit, selectId).then((response) => {
          if (response?.status === 200) {
            getData();
            closePopUp();
          }
    ...
    <div className={styles.DirectoryLegalEntities}>
      <div className={styles.DirectoryLegalEntitiesTop}>
        ...
        <div className={styles.DirectoryLegalEntitiesTopButton}>
          ...
          <button onClick={() => EditLegalEntitiesFinc()}>
      ...
      <UniversalTable
        tableHeader={tableLagealEntries}
        tableBody={tableDataEntries}
        selectFlag={true}
        tableName="table1"
      />
      {deleteUnitFlag && (
