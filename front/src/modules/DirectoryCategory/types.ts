...
export interface CreateDirectoryCategoryPayload extends Pick<
...
export interface UpdateDirectoryCategoryBodyPayload extends Pick<
  CreateDirectoryCategoryPayload,
  'name' | 'color' | 'builderId' | 'customersIds' | 'isExternal' | 'builderExternalId'
> {}
...
export interface GetAllBuildersResponse extends Pick<Option, 'value' | 'label'> {
  isExternal: boolean;
  isManager: boolean;
}
...
export interface NormalizedDataI extends Pick<
  GetDirectoryCategoryResponse,
  'name' | 'number' | 'color'
> {
