...
import { DirectoryCategoryModalFormI, DirectoryCategoryModalProps } from '../types';
...
const DirectoryCategoryModal: FC<DirectoryCategoryModalProps> = ({
  ...
  const { register, handleSubmit, control, reset } = useForm<DirectoryCategoryModalFormI>();
    ...
    if (state.open) {
      if (state.type === 'edit' && selectedRow) {
        ...
        reset({
          ...
          customersIds: selectedRow.customers?.map((c) => c.id) ?? [null],
          builder: builder?.id ?? '',
  ...
  const onSubmit: SubmitHandler<DirectoryCategoryModalFormI> = async (data) => {
    const builder = allBuildersOptions?.find((builder) => builder.value === data.builder);
    const payload = {
      name: data.name,
      color: data.color,
      builderId: !builder?.isManager && !builder?.isExternal ? data.builder : null,
      customersIds:
        (data.customersIds && data.customersIds[0] === null) || data.customersIds === null
          ? null
          : data.customersIds,
      isExternal: builder?.isExternal ?? false,
      managerId: builder?.isManager && !builder?.isExternal ? data.builder : null,
      builderExternalId: !builder?.isManager && builder?.isExternal ? data.builder : null,
  ...
  useEffect(() => {
    if (context?.settingsList.length > 0) {
      ...
      if (auto && typeof auto.value === 'boolean') setAutoSetting(auto.value);
    }
  }, [context?.settingsList]);

  return (
    <Modal
      title={state.type === 'add' ? 'Добавление категории' : 'Редактирование категории'}
      open={state.open}
      onClose={handleClose}
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className={styles.color__container}>
          ...
          <input {...register('color')} type="color" className={styles.color} />
        </div>
