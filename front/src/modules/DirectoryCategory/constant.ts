...
export const normalizeDataRender = (
    data: GetDirectoryCategoryResponse[]
): NormalizedDataI[] => {
    ...
    data.map((record) => {
        ...
        if (!record?.isExternal && record.builder)
            builder = record.builder?.name;
        if (record.isExternal && record.builderExternal)
            builder = record.builderExternal?.name;
