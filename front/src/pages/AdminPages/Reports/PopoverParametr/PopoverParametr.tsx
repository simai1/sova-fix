...
const PopoverParametr: FC<PopoverParametrProps> = ({
    ...
    const selectedValues = useMemo(() => {
        if (
            ...
            !(parametr in (filterDataValues ?? {}))
        ) {
            return checkBoxes.map((cb) => cb.value);
        }

        const values =
            filterDataValues[parametr as keyof typeof filterDataValues];
            ...
            <div className={styles.container}>
                {checkBoxes?.length > 0 ? (
                    <Checkbox.Group
                        ...
                        onChange={(vals) =>
                            handleSetFilterDataValues(parametr, vals)
                        }
                    />
                ) : (
                    <Typography.Paragraph className={styles.text}>Для фильтрации отметьте данный параметр</Typography.Paragraph>
                )}
            </div>
        </ConfigProvider>
    );
