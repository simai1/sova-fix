...
import { additionalParametrsSelector, tableReportDataSelector } from '../../selectors';
    ...
    <table className={styles.table}>
      ...
      <tbody>
        {table.getRowModel().rows.map((row) => {
          const firstKey = Object.keys(row.original)[0] as keyof ReportTableI;
          const isTotalRow =
            Object.values(row.original).some((v) => v === 'Итого') ||
            row.original[firstKey] === 'Итого';

          return (
            <tr
              key={row.id}
              className={`${isResult ? styles.trTable : ''} ${isTotalRow ? styles.totalRow : ''}`}
            >
