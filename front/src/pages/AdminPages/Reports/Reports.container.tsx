...
const ReportsContainer: FC = () => {
  ...
  const [isReloadButtonLoading, setIsReloadButtonLoading] = useState<boolean>(false);
  ...
  const [getTableReportData, { isFetching: isLoadingTableData }] = useLazyGetTableReportDataQuery();
  ...
  const isDisabledIndicators = (name: string): boolean => {
    ...
    if ((name === 'budgetPlan' || name === 'percentOfBudgetPlan') && !parametrs?.object) {
      return true;
    }
  ...
  useEffect(() => {
    if (setIsReloadButtonLoadingAction) {
      dispatch(setIsReloadButtonLoadingAction(isReloadButtonLoading ?? false));
    }
  }, [isReloadButtonLoading, dispatch]);

  const handleSetFilterDataValues = (fieldName: keyof ReportTable, values: string[]) => {
    if (setFilterDataValues) {
      dispatch(
        setFilterDataValues({
          ...filterDataValues,
  ...
  const onValuesParametersChange = (changedValues: ParametrsFormInstance) => {
    const key = Object.keys(changedValues)[0] as keyof ParametrsFormInstance;
  ...
  const onValuesIndicatorsChange = (changedValues: IndicatorsFormInstance) => {
    if (!isGraphicType) return;
    const key = Object.keys(changedValues)[0] as keyof IndicatorsFormInstance;
    const value = changedValues[key];
