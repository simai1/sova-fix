...
import { Flex, DatePicker, ConfigProvider, Select, Form, Dropdown, TreeSelect } from 'antd';
...
const AdditionalParametrs: FC<AdditionalParametrsProps> = ({
  ...
  useEffect(() => {
    ...
    form.setFieldValue('periodPicker', start && end ? [start, end] : undefined);
        ...
        <Flex vertical gap={10} className={styles.content}>
          <Flex gap={10}>
            <AntButton
              ...
              loading={isReloadButtonLoading ? { icon: <SyncOutlined spin /> } : false}
            >
              Обновить
            </AntButton>
            <AntButton colorVariant="brown" onClick={handleResetFiltersWithAdditionals}>
              Очистить отчёт
            </AntButton>
          </Flex>
          <Form
          ...
          >
            <Flex align="center" gap={20}>
              <Form.Item<AdditionalParametrsForm> noStyle name="periodPicker">
                <RangePicker
              ...
              <Form.Item<AdditionalParametrsForm> noStyle name="periodDropdown">
                <Select
              ...
              <div className={styles.selectContainer}>
                {!isGraphic && (
                  <Form.Item<AdditionalParametrsForm> name="dynamicsTypes" noStyle>
                    <TreeSelect
            ...
            <Flex gap={10}>
              <Form.Item<AdditionalParametrsForm> name="reportType" noStyle>
                <Select placeholder="Тип отчета" className={styles.select} options={REPORT_TYPES} />
              </Form.Item>
              <Dropdown
                trigger={['click']}
                menu={{
                  items: [
                    {
                      key: 'xlsx',
                      label: <div onClick={() => exportToExcel(tableReportData)}>xlsx</div>,
                    },
                  ],
                }}
              >
                <AntButton colorVariant="brown">Экспорт</AntButton>
              </Dropdown>
            </Flex>
          </Form>
        </Flex>
