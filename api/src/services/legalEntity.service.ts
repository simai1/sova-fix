...
const createLegalEntity = async (
    ...
    const legalEntity = await LegalEntity.create({ name, legalForm, count, startCoop: startCoopDate, number: 1 }, {});
...
const updateLegalEntity = async (
    ...
    const unit = await checkLegalEntity.update({ name, legalForm, count, startCoop: startCoopDate });
