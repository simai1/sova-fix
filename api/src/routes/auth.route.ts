...
import { registerPublicSchema } from '../validations/auth.validation';

const router = Router();

router.route('/register').post(authController.registerViaEmail);
router.route('/register-public').post(validator(registerPublicSchema), authController.registerPublic);
